...
export type CurrentHoldingsOptions = {
  account?: string
...
async function fetchFxSnapshotIfNeeded(
  groups: HoldingGroup[],
  forceRefresh: boolean
) {
  ...
  try {
    return forceRefresh
      ? await fetchUsdTwdFxSnapshot(fetch, { forceRefresh: true })
      : await fetchUsdTwdFxSnapshot()
  } catch {
    return null
  }
}
...
export async function getCurrentHoldings({
  ...
  const quotes = forceRefresh
    ? await fetchPreviousCloseSnapshots(quoteTargets, fetch, {
        forceRefresh: true,
      })
    : await fetchPreviousCloseSnapshots(quoteTargets)
  ...
  const fxSnapshot = await fetchFxSnapshotIfNeeded(matchedGroups, forceRefresh)
  ...
  return {
    ...
    totalHoldings: countHoldings(filteredGroups),
    issues,
  }
}
